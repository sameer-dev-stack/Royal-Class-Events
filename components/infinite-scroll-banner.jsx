...
export default function InfiniteScrollBanner() {
    ...
    return (
        <div className="w-full overflow-hidden bg-amber-500/5 border-y border-amber-500/10 py-4 md:py-6 backdrop-blur-sm relative z-20">
                ...
                <div className="flex items-center flex-nowrap shrink-0 gap-10 md:gap-16 px-4 md:px-8">
                    {items.map((item, index) => (
                        <div key={`a-${index}`} className="flex items-center gap-4 md:gap-6 whitespace-nowrap">
                            <span className="text-xl md:text-2xl font-black tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-amber-600/40 to-amber-400/40">
                                {item}
                            </span>
                            <Star className="w-3 h-3 md:w-5 md:h-5 text-amber-500/30" />
                        </div>
                    ))}
                </div>
