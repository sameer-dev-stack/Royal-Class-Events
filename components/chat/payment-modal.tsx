...
export function PaymentModal({
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && !isSubmitting && onClose()}>
            <DialogContent className="bg-zinc-950 border-zinc-800 text-white max-w-md rounded-[2rem] overflow-hidden p-0 gap-0 shadow-2xl">
                ...
                <div className="px-8 py-6 space-y-6">
                    ...
                    <div className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-2xl space-y-4">
                        <div className="flex justify-between items-center">
                            <span className="text-zinc-500 text-sm font-medium uppercase tracking-wider">Service</span>
                            <span className="text-white font-bold text-sm line-clamp-1 max-w-[200px]">{offerTitle}</span>
                        </div>
                        <div className="h-px bg-zinc-800" />
                        <div className="flex justify-between items-end">
                            <span className="text-zinc-500 text-sm font-medium uppercase tracking-wider">Total to Pay</span>
                            <span className="text-3xl font-black text-white">{formatPrice(amount)}</span>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex items-center gap-3 p-3 rounded-xl bg-zinc-900/30 border border-zinc-800/50">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-tighter text-zinc-500">Encrypted</span>
                        </div>
                        <div className="flex items-center gap-3 p-3 rounded-xl bg-zinc-900/30 border border-zinc-800/50">
                            <CreditCard className="w-4 h-4 text-zinc-500" />
                            <span className="text-[10px] font-bold uppercase tracking-tighter text-zinc-500">Secure Vault</span>
                        </div>
                    </div>
                </div>
